...
public class FissionSFRConfiguration extends AbstractBlockContainer<Block> {
    ...
    public void apply(FissionSFRConfiguration partial, ArrayList<Multiblock> multiblocks, PartialConfiguration parent){
        ...
        for(Multiblock mb : multiblocks){
            if(mb instanceof OverhaulSFR){
                for(net.ncplanner.plannerator.multiblock.overhaul.fissionsfr.Block b : ((OverhaulSFR)mb).getBlocks()){
    ...
    public Block convertToSFR(net.ncplanner.plannerator.multiblock.configuration.overhaul.fissionmsr.Block template) throws MissingConfigurationEntryException{
        if(template==null)return null;
        for(Block block : allBlocks){
            if(block.name.equals(Core.superReplace(template.name, "salt", "solid", "vessel", "cell", "heater", "sink")))return block;
        }
        for(Block block : blocks){
            if(block.name.equals(Core.superReplace(template.name, "salt", "solid", "vessel", "cell", "heater", "sink")))return block;
        }
