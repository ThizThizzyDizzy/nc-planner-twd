...
public class OverhaulModeratorLineDecal extends Decal{
    ...
    public void render(Renderer renderer, double x, double y, double blockSize){
        ...
        renderer.setColor(Core.theme.getDecalTextColor());//TODO proper font settings
        switch(direction){
            case PX:
                renderer.drawCenteredText(-blockSize+x, y+blockSize*.55, blockSize+x+blockSize, flux+"");
                break;
            case NX:
                renderer.drawCenteredText(-blockSize+x, y+blockSize*.25, blockSize+x+blockSize, flux+"");
                break;
            case PY:
                renderer.drawCenteredText(-blockSize+x+blockSize*.55, y+blockSize*.25, blockSize+x+blockSize*.75, flux+"");
                break;
            case NY:
                renderer.drawCenteredText(-blockSize+x+blockSize*.25, y+blockSize*.55, blockSize+x+blockSize*.45, flux+"");
                break;
            case PZ:
                renderer.drawCenteredText(-blockSize+x+blockSize*.25, y+blockSize*.4, blockSize+x+blockSize*.45, flux+"");
                break;
            case NZ:
                renderer.drawCenteredText(-blockSize+x+blockSize*.55, y+blockSize*.4, blockSize+x+blockSize*.75, flux+"");
                break;
        }
    }
    @Override
